...
class Transformation:
    ...
    def plot_image(self, image, transformation):
        ...
        if self.debug == "print":
            pcv.print_image(
                image,
                os.path.join(self.outdir, f"{self.image_name}_{transformation}.jpg"),
            )
        else:
    ...
    def roi(self):
        ...
        kept_mask = pcv.roi.filter(mask=self._plant_mask, roi=roi, roi_type="partial")
    ...
    def pseudolandmarks(self):
        if self.transformation in ("all", "pseudolandmarks") and self.debug in (
            "plot",
            "print",
        ):
    ...
    def colors(self):
        ...
        pcv.analyze.color(
            rgb_img=self.image, colorspaces="all", labeled_mask=mask, label="plant"
        )
        ...
        if self.transformation in ("all", "colors") and self.debug in ("plot", "print"):
            if self.debug == "plot":
                ...
                plt.savefig(os.path.join(self.outdir, f"{self.image_name}_colors.jpg"))
...
def parser():
    ...
    parser.add_argument(
        "path", nargs="?", type=str, help="path to the image for transformation"
    )
    parser.add_argument(
